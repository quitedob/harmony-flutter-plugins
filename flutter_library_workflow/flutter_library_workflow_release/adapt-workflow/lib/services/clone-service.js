...
/**
 * Convert GitHub HTTPS URL to SSH if no token is configured
 * @param {string} url - Original URL (HTTPS or SSH)
 * @param {object} settings - Settings object containing githubToken
 * @returns {string} SSH URL if applicable, otherwise original URL
 */
function convertToSshIfNeeded(url, settings) {
  if (!url || !url.startsWith('https://github.com/')) {
    return url;
  }

  // If token is configured, keep HTTPS with token auth
  if (settings.githubToken) {
    return url.replace('https://github.com/', `https://${settings.githubToken}@github.com/`);
  }

  // No token, convert to SSH
    ...
    try {
      if (branch === targetBranch) {
        ...
        if (fsSync.existsSync(repoPath)) {
          ...
          // Convert to SSH if no GitHub token configured
          const cloneUrl = convertToSshIfNeeded(plugin.repoUrl, settings);
...
async function syncToTpc(pluginId) {
  ...
  let pushUrl = tpcUrl;
  // 支持 SSH 方式：如果 tpcUrl 是 HTTPS 且没有配置 githubToken，自动转换为 SSH
  if (tpcUrl.startsWith('https://github.com/')) {
    if (settings.githubToken) {
      // 使用 Token 认证的 HTTPS
      pushUrl = tpcUrl.replace('https://github.com/', `https://${settings.githubToken}@github.com/`);
    } else {
      // 没有 Token 时使用 SSH
      pushUrl = tpcUrl.replace('https://github.com/', 'git@github.com:').replace(/\.git$/, '') + '.git';
    }
  }
