...
本清单面向 Flutter 插件鸿蒙适配场景。通用 ArkTS 语法约束由 `arkts-rules` Skill 覆盖，本文件聚焦适配代码的高频质量问题。
...
| 假实现模式 | 规则 | 判定标准 |
|-----------|------|---------|
| `result.success('')` | `cr-no-fake-empty-string` | 空字符串作为占位返回 |
| `result.success(false)` | `cr-no-fake-false` | 布尔 `false` 作为占位返回 |
| `result.success(0)` | `cr-no-fake-zero` | 数值 `0` 作为占位返回 |
| `result.success(new Map())` | `cr-no-fake-empty-map` | 空 Map 作为占位返回 |
| `result.success(new HashMap())` | `cr-no-fake-empty-hashmap` | 空 HashMap 作为占位返回 |
| `result.success([])` | `cr-no-fake-empty-array` | 空数组作为占位返回 |
| 方法体只有 `hilog` + `result.success` | `cr-no-log-only-impl` | 无真实系统 API 或三方库 API 调用 |

检测方式：逐方法扫描 MethodChannel handler 中 `case 'xxx':` 或 `if (call.method === 'xxx')` 分支的方法体，检查是否存在真实 API 调用。仅有 `hilog` 和 `result.success(占位值)` 的视为假实现。

### not_implemented 一致性

| 违规模式 | 规则 | 优先级 |
|---------|------|--------|
| JSON 声明 `not_implemented` 但代码返回 `result.success(...)` | `cr-notimpl-returns-success` | P0 |
| JSON 声明 `implemented` 但代码返回 `result.notImplemented()` | `cr-impl-returns-notimpl` | P0 |
| 代码路径返回 `result.error()` 但未提供 error code 或 message | `cr-error-missing-info` | P1 |
...
| `catch` 块为空或只有 `hilog` | `cr-catch-must-respond` | catch 中必须调用 `result.error()` 将错误传回 Dart 层 |
| `Promise` 无 `await` 也无 `.catch()` | `cr-no-fire-and-forget` | 禁止 fire-and-forget，异步错误必须被捕获 |
| 异步回调中未检查组件生命周期 | `cr-async-lifecycle-check` | 回调执行时组件可能已 detach，修改状态前须检查 |

检测方式：搜索 `async` 方法中的 `await` 调用，确认被 `try-catch` 包裹；搜索 `catch` 块确认包含 `result.error`
...
| EventChannel 回调与 MethodChannel handler 竞态 | `cr-event-method-race` | EventChannel 的 sink 操作与 MethodChannel 的状态修改可能并发 |
...
在 `onAttachedToEngine` 中注册的所有资源，必须在 `onDetachedFromEngine` 中有对应释放：

| 注册操作（onAttachedToEngine） | 释放操作（onDetachedFromEngine） | 规则 |
|-------------------------------|--------------------------------|------|
| `MethodChannel` + `setMethodCallHandler` | `channel.setMethodCallHandler(null)` 或 `channel = null` | `cr-release-method-channel` |
| `EventChannel` + `setStreamHandler` | `eventChannel.setStreamHandler(null)` 或 `eventChannel = null` | `cr-release-event-channel` |
...
检测方式：
1. 在 `onAttachedToEngine` / `onAttachedToAbility` 中提取所有注册操作
2. 在 `onDetachedFromEngine` / `onDetachedFromAbility` 中检查对应释放
3. 报告不对称的注册/释放对
...
| 检查项 | 规则 | 说明 |
|--------|------|------|
| 实现 `AbilityAware` 但 `onDetachedFromAbility` 未清空 context | `cr-ability-detach-context` | 必须在 detach 时将 `UIAbilityContext` 引用置 null |
| detach 后继续使用 `UIAbilityContext` | `cr-ability-use-after-detach` | detach 后 context 已失效，使用会崩溃 |
| 使用 `getApplicationContext()` 强转 `UIAbilityContext` | `cr-no-appctx-cast-uiability` | 禁止，必须通过 `AbilityAware.onAttachedToAbility()` 获取 |
...
| 使用了 `requestPermissionsFromUser` 但未实现 `AbilityAware` | `cr-perm-no-ability-aware` | `requestPermissionsFromUser` 需要 `UIAbilityContext`，必须通过 `AbilityAware` 获取 |
...
| 检查项 | 规则 | 说明 |
|--------|------|------|
| TAG 未使用插件名 | `cr-hilog-tag-plugin-name` | TAG 必须使用插件名（如 `'flutter_blue'`），禁止 `'test'` / `'debug'` / `'TAG'` |
| 打印敏感信息 | `cr-hilog-no-sensitive` | 禁止打印 token、password、密钥、用户 ID、完整文件内容 |
| Channel 方法入口无日志 | `cr-hilog-entry-log` | 每个 Channel 方法入口应有 `hilog.debug(...)` |
| 错误路径无日志 | `cr-hilog-error-log` | `catch` / `result.error` 路径应有 `hilog.error(...)`，含错误码和消息 |
...
| 检查项 | 规则 | 说明 |
|--------|------|------|
| `result.success` / `result.error` 返回未脱敏系统信息 | `cr-no-leak-system-info` | 禁止返回设备 ID、完整文件路径、系统版本等未脱敏数据 |
| 外部输入未校验直接使用 | `cr-validate-external-input` | Channel 接收的参数使用前必须检查类型和范围 |
| 异常信息包含敏感数据 | `cr-no-sensitive-in-error` | `result.error()` 的 message 不应包含用户凭据、文件内容 |
...
| 元素 | 风格 | 规则 | 示例 |
|------|------|------|------|
| 文件名 | kebab-case 或下划线 | `cr-file-naming` | `xxx-plugin.ets`、`xxx_plugin.ets` |
| 类名、枚举名、命名空间名、构造器函数 | UpperCamelCase | `cr-class-naming` | 类 `FlutterBluePlugin`、枚举 `UserType`、命名空间 `Base64Utils` |
| 方法名、变量名、参数名 | lowerCamelCase | `cr-method-naming` | 方法 `handleMethodCall`、变量 `channelName`、参数 `filePath` |
...
| Web mixedMode 设为 All（仅 Web 插件） | `cr-web-mixed-mode` | `mixedMode(MixedMode.All)` |
| Web fileAccess 未关闭（仅 Web 插件） | `cr-web-file-access` | `fileAccess(true)` 或未设置 |
| Web geolocationAccess 未关闭（仅 Web 插件） | `cr-web-geolocation` | `geolocationAccess(true)` 或未设置 |
| HTTP 正则校验缺 ^ 锚点 | `cr-url-regex-anchor` | `http(s)?:` 正则无 `^` |
| Web 存储敏感数据（仅 Web 插件） | `cr-web-sensitive-storage` | localStorage/sessionStorage + token/password |
...
| 假实现 | `grep "result.success(''" / result.success(false)` | 接入真实 API 或标记 `notImplemented()` |
| catch 吞异常 | `grep 'catch'` + 检查块内容 | 补 `result.error(code, msg, details)` 回传 |
| 资源未释放 | 对比 `onAttach` vs `onDetach` | 补释放逻辑 |
