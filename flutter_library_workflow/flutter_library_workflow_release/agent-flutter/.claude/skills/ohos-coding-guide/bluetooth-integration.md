...
适用于 Flutter 插件适配 HarmonyOS 时涉及：
...
`entry/src/main/module.json5` 或插件 example 的宿主 HAP 里必须声明：
...
- 只让 Example 页面启动时触发一次蓝牙 API
- 假设系统会因为 API 调用自动弹窗

必须在插件库内部基于真实 `UIAbilityContext` 调用 `requestPermissionsFromUser(...)`。
...
Flutter 插件里通常在 `onAttachedToAbility` 保存上下文，所有蓝牙入口都先检查权限。
...
export default class BluetoothPlugin implements AbilityAware {
...
可优先用 `access.enableBluetoothAsync()` / `access.disableBluetoothAsync()` 以 Promise 形式拿到底层错误。低版本用 `enableBluetooth()` 后通过 `stateChange` 判断结果。
...
## 2. 先按 Flutter 公开语义选择传统蓝牙或 BLE

不要看到 `scan` / `search` / `connect` 就直接写 BLE。蓝牙插件适配必须先读 Dart API 和 Example，确认公开语义属于哪条官方链路。

| Flutter 公开语义 | 官方文档主题 | HarmonyOS API |
| --- | --- | --- |
| 蓝牙开关、状态 | 蓝牙设置 | `access` |
...
适配 `bondedDevices`、`pairedDevices`、`getConnectedDevices` 等 Dart 语义时，不要用主动扫描冒充已配对列表。
...
async function connectAllowedProfiles(deviceId: string): Promise<void> {
  ...
  const hid = constant.ProfileUuids.PROFILE_UUID_HID.toString().toLowerCase();
...
export class SppClientBridge {
  ...
  private onRead = (buffer: ArrayBuffer): void => {
    ...
    // 这里应转发到 Dart：EventChannel / MethodChannel 回调 / 状态流。
  };
...
- Dart `Uint8List` 到 ArkTS 通常按 `ArrayBuffer` 处理；回传 Dart 也应保持字节语义。
...
- 如果 Flutter API 期望持续扫描结果，应通过 `EventChannel` 回传。
...
- 监听到 `BLECharacteristicChange` 后，要通过 Dart 公开通道回传，不要只写日志。

## 8. Flutter 通道映射

蓝牙原生监听到了，不等于 Flutter 插件能力完成。只要 Dart 公开 API 包含以下语义，就必须回传：
...
| 原生事件 | Flutter 通道 |
| --- | --- |
| 扫描结果持续上报 | `EventChannel` |
| 连接状态流 | `EventChannel` |
| SPP/GATT 收到 bytes | `EventChannel` |
| 一次性操作，如 start/stop/connect/write | `MethodChannel` |
...
不要把所有失败都压成 `false`。至少在原生层日志和 Dart 错误码里区分：
...
如果原 Flutter 插件 API 只能返回 bool，也建议：

- hilog 里保留细分原因
- 内部状态保留细分错误
- 能走 `result.error(...)` 时保留错误码
...
- `onDetachedFromEngine`：停止扫描、取消 EventChannel、关闭 socket/gatt、清空 maps。
- `onDetachedFromAbility`：清空 `UIAbilityContext`，不要继续请求权限。
- `onCancel`：取消当前 EventChannel 对应的原生监听。
- 重复 start/connect 前：先检查旧状态，必要时 stop/close 旧资源。

## 11. 最低自检项

- 是否声明 `ohos.permission.ACCESS_BLUETOOTH`
- 是否在插件库内部调用 `requestPermissionsFromUser(...)`
