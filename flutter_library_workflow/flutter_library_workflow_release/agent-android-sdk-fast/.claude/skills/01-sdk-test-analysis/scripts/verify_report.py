...
def verify_table_headers(report_content: str) -> tuple[bool, list[str]]:
    ...
    required_tables = {
        ...
        '3.1 功能点表': r'\|\s*用例编号\s*\|\s*功能模块\s*\|\s*功能点\s*\|\s*验证点\s*\|\s*覆盖 API\s*\|\s*级别\s*\|',
        '3.1.1 功能点汇总表': r'\|\s*功能模块\s*\|\s*优先级\s*\|\s*功能点数量\s*\|\s*覆盖的主要 API\s*\|',
...
def verify_ibo_format(report_content: str) -> tuple[bool, list[str]]:
    ...
    # 检查模块标题格式：只要求以 #### F-数字 开头，后面有标题文本；
    # 标题任意位置都可以带中文括号或英文括号，不限制括号内容。
    module_pattern = r'^####\s+F-\d+\s+\S.*$'
    modules = re.findall(r'^####\s+F-\d+.*$', report_content, re.MULTILINE)
