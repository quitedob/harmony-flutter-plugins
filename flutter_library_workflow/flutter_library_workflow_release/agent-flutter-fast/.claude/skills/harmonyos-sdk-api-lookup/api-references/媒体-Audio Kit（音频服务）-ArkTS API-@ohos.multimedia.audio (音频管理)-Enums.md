...
| INTERRUPT\_SHARE | 1 | 共享打断类型，即系统不执行具体操作，通过[InterruptHint](/consumer/cn/doc/harmonyos-references/arkts-apis-audio-e#interrupthint)建议并提示应用操作，应用可自行决策下一步处理方式。 |
...
| INTERRUPT\_HINT\_RESUME | 1 | 提示音频恢复，应用可主动触发开始渲染或开始采集的相关操作。  此操作无法由系统强制执行，其对应的[InterruptForceType](/consumer/cn/doc/harmonyos-references/arkts-apis-audio-e#interruptforcetype9)一定为INTERRUPT\_SHARE类型。  **元服务API：** 从API version 12开始，该接口支持在元服务中使用。 |
...
| PRIVACY\_TYPE\_SHARED21+ | 2 | 表示音频流可以被其他应用录制或屏幕投射，包含隐私类型的流。  例如，在PRIVACY\_TYPE\_PUBLIC策略下，[STREAM\_USAGE\_VOICE\_COMMUNICATION](/consumer/cn/doc/harmonyos-references/arkts-apis-audio-e#streamusage)类型音频流不会被其他应用录制或屏幕投射。  然而，在PRIVACY\_TYPE\_SHARED策略下，这些音频流将会允许被其他应用录制或屏幕投射。 |
...
| SOURCE\_TYPE\_PLAYBACK\_CAPTURE(deprecated) | 2 | 播放音频流（内录）录制音频源。  **系统能力：** SystemCapability.Multimedia.Audio.PlaybackCapture  从API version 10开始支持，从API version 12开始废弃，建议使用[录屏接口AVScreenCapture](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-avscreencapture)替代。 |
| SOURCE\_TYPE\_VOICE\_COMMUNICATION | 7 | 语音通话场景的音频源（单独启动录制不会开启3A算法，需同时使用[STREAM\_USAGE\_VOICE\_COMMUNICATION](/consumer/cn/doc/harmonyos-references/arkts-apis-audio-e#streamusage)或[STREAM\_USAGE\_VIDEO\_COMMUNICATION](/consumer/cn/doc/harmonyos-references/arkts-apis-audio-e#streamusage)类型的AudioRender起播才会触发开启3A算法）。  **系统能力：** SystemCapability.Multimedia.Audio.Core |
...
| AUDIO\_SESSION\_STATE\_CHANGE\_HINT\_DUCK | 4 | 提示音频会话躲避开始，降低音量播放。  如果已启用[enableMuteSuggestionWhenMixWithOthers](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-audio-audiosessionmanager#enablemutesuggestionwhenmixwithothers23)，此时可以选择执行静音操作。 |
| AUDIO\_SESSION\_STATE\_CHANGE\_HINT\_UNDUCK | 5 | 提示音频会话躲避结束，恢复音量播放。  如果已启用[enableMuteSuggestionWhenMixWithOthers](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-audio-audiosessionmanager#enablemutesuggestionwhenmixwithothers23)，此时可取消静音。 |
...
| AUDIO\_SESSION\_STATE\_CHANGE\_HINT\_MUTE24+ | 8 | 提示音频会话静音。  该提示仅在以下条件满足后才会收到：通过接口[setAudioSessionBehavior](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-audio-audiosessionmanager#setaudiosessionbehavior24)设置参数[AudioSessionBehaviorFlags](/consumer/cn/doc/harmonyos-references/arkts-apis-audio-e#audiosessionbehaviorflags24).MUTE\_WHEN\_INTERRUPTED，并已调用[setAudioSessionScene](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-audio-audiosessionmanager#setaudiosessionscene20)，且音频会话已激活。  **模型约束：** 此接口仅可在Stage模型下使用。 |
| AUDIO\_SESSION\_STATE\_CHANGE\_HINT\_UNMUTE24+ | 9 | 提示音频会话解除静音，恢复播放。  该提示仅在以下条件满足后才会收到：通过接口[setAudioSessionBehavior](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-audio-audiosessionmanager#setaudiosessionbehavior24)设置参数[AudioSessionBehaviorFlags](/consumer/cn/doc/harmonyos-references/arkts-apis-audio-e#audiosessionbehaviorflags24).MUTE\_WHEN\_INTERRUPTED，并已调用[setAudioSessionScene](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-audio-audiosessionmanager#setaudiosessionscene20)，且音频会话已激活。  **模型约束：** 此接口仅可在Stage模型下使用。 |
...
从API version 7开始支持，从API version 9开始废弃，建议使用[InterruptType](/consumer/cn/doc/harmonyos-references/arkts-apis-audio-e#interrupttype)替代。
...
| MUTE\_WHEN\_INTERRUPTED24+ | 0x00000002 | 当音频流被打断时，使用静音替代。  通过接口[setAudioSessionBehavior](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-audio-audiosessionmanager#setaudiosessionbehavior24)设置该行为的同时，也需要调用接口[setAudioSessionScene](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-audio-audiosessionmanager#setaudiosessionscene20)使其生效。  当播放被静音时，应用将收到[InterruptHint](/consumer/cn/doc/harmonyos-references/arkts-apis-audio-e#interrupthint).INTERRUPT\_HINT\_MUTE通知，并且在恢复时会收到[InterruptHint](/consumer/cn/doc/harmonyos-references/arkts-apis-audio-e#interrupthint).INTERRUPT\_HINT\_UNMUTE通知。 |