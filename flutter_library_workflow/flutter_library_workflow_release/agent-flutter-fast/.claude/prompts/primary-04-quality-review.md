...
- **评估对象 = Flutter 插件的鸿蒙库本体 `ohos/`**（本工作流交付的鸿蒙侧成果物，含 `oh-package.json5`、`src/main/ets/`、`Index.ets`/入口、`module.json5`、`build-profile.json5`）；要回答的是「这个鸿蒙库自身写得好不好」。
- **本阶段不是集成评估**：不评估 Example（`example/`、`example/ohos/`），不评估「库能否集成进宿主 App / 能否在 Example 里跑通」，也不重复校验与原生端（Android/iOS）的功能对齐——功能、集成与编译已在「代码编写」「编码校验」阶段完成并修复。
...
- 被评估的鸿蒙库根目录 **`ohos/`**（含 `oh-package.json5`、`Index.ets`/入口、`module.json5`、`build-profile.json5`）——**唯一的评估对象**。
- `.ohos-adaptation/03-validation-report.md`（仅作背景参考，了解编码校验阶段的结论与已修复项；不据此重复做集成 / 功能校验）

> 评估范围严格限定在插件的 `ohos/` 鸿蒙库，**不纳入** `example/`（含 `example/ohos/`，Demo）的代码——Example 只是验收载体，其质量不属于本阶段评估范围。
...
使用 **`hmos-library-quality-assessment`** Skill，对 `ohos/` 鸿蒙库执行白盒质量评估：

1. **阶段0 库画像**：识别库名/版本、产物类型、导出/依赖、规模，填入报告头（**不判定 UI/逻辑/NAPI 库型**，不展开 UI 专项维度）。
2. **阶段1 静态扫描（CodeLinter，硬前置）**：探测 DevEco Studio CodeLinter，**仅对 `ohos/` 鸿蒙库模块**做全量规则集扫描（不加 `--fix`、不扫 `example/`），按级别 + 规则集汇总为独立报告节。
3. **阶段2 通用架构评估**：A1 通用架构（SOLID / 耦合内聚 / 反模式 / 分层依赖方向）。
4. **阶段3 代码质量**：B1 通用·逻辑（ArkTS 规范 / 复杂度 / 错误处理 / 逻辑性能 / 加密安全）。
5. **阶段4 DFX 质量检测**：使用 **`dfx-quality`** Skill 的检测脚本（`--dry-run` 模式）对 `ohos/` 和 `lib/` 做自动化扫描，再对脚本无法覆盖的问题由 agent 逐项核对。C1 稳定性 / C2 性能 / C3 功耗 / C4 UX / C5 兼容性 适用于所有插件，检查项与命令详见 `dfx-quality` Skill。
...
3. 架构发现（A1 通用架构）
4. 代码质量发现（B1）
5. DFX 质量检测发现（C1 稳定性；C2 性能；C3 功耗；C4 UX；C5 兼容性）
6. 修复优先级清单（含 DFX 发现）
7. CodeLinter 扫描结果（独立节·按级别 + 规则集·不拆维度；若 CodeLinter 缺失则注明已跳过）

> 全程不修改 `ohos/`，CodeLinter 配置与输出均放在 scratch 目录。
