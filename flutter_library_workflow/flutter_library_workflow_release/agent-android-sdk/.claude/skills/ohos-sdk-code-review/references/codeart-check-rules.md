...
审查对象：`library/src/main/ets/` 下本阶段新增或修改的 `.ets` / `.ts` 文件。
...
- **说明**：`JSON.parse()` 解析外部入参（Want 参数、宿主传入参数等）可能抛异常，必须 try-catch 捕获。`JSON.stringify()` 对含循环引用的对象也会抛异常。
...
> 注：`cr-release-*` 系列规则覆盖 ArkUI 生命周期 / Listener / Timer 等资源的对称释放，本规则专门覆盖文件 I/O 资源。
...
grep -nE '\[[0-9]+\]' {files}
# e) 十六进制字面量（如 hilog domain 0x0000）
...
# g) 比较表达式中的数值字面量（如 error.code === 1006800010, length === 0, >= 0）
grep -nE '(===?|!==?|>=?|<=?)\s*-?[0-9]+' {files} | grep -v 'import\|from\|//'
# h) switch case 中的数值字面量（如 case 4:）
grep -nE '^\s*case\s+-?[0-9]+' {files}
# i) Map.set / 函数调用的末位数值参数（如 set('errorCode', 401), slice(0, length)）
grep -nE ",\s*-?[0-9]+\s*\)" {files} | grep -v 'import\|from\|//'
```

> a/b 扫定义点，c/d/e/f 扫高频遗漏的用法点（三元数值、数组下标、十六进制字面量、函数参数数值），g/h/i 扫比较表达式、switch case、函数末位参数。阅读代码时再找出 grep 未覆盖的遗漏魔法值（赋值/运算/返回值等），全文替换为常量引用。**清单中的每个魔法值都必须修复，不得以任何理由保留字面量。** 详见 SKILL.md §4.1 第 6 组的修复流程。
