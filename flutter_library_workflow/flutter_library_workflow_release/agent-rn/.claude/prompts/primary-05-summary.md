...
- `.rn-ohos-adaptation/04-testing.json` — Example 编译状态、方法覆盖率（example生成）
- `.rn-ohos-adaptation/04-device-verify.json` — 运行态检测、设备验证、库修复记录（测试验证；缺失时兼容旧版合并在 04-testing 的设备字段）
...
**从 04-testing.json（example生成）：**
- `example_build_status`
- `method_coverage`（Example 对已实现方法的覆盖率）
- `compilation_fixes` 的数量（若有）

**从 04-device-verify.json（测试验证；若无则回退读 04-testing.json 中同名字段）：**
...
**重要**：API 实现了代码不代表真正支持！撰写评分依据、`INTEGRATION_GUIDE.md` 与报告中的 API 支持描述时，须结合鸿蒙 SDK 调研与 04 设备/静态检测结果（**不在本阶段修改 `ohos/README.md`**，README 由 04-testing 负责）。

**调研流程**：

```
对每个 implemented_method:
  1. 从 02-planning.json 的 ohos_api_mapping 找到对应的鸿蒙 API
  2. 加载 harmonyos-sdk-api-lookup Skill 搜索该 API
  3. 检查 API 文档/类型定义：
     - API 是否存在？
     - 功能是否完整？
     - 是否有平台限制（如仅特定设备、特定版本）？
     - 是否需要硬件支持（如 Telephony、NFC）？
  4. 根据调研结果 + 04-device-verify.json（或旧版 04-testing.json）的 device_test_results 判定支持状态
```
...
| 状态 | 判定条件 |
|------|----------|
| `✅ 完全支持` | 鸿蒙 API 存在 + 功能完整 + 设备测试 pass + 无风险 |
| `⚠️ 部分支持` | 鸿蒙 API 存在但：功能不完整 / 有平台限制 / 有 medium 风险 |
| `⚠️ 返回值差异` | 鸿蒙 API 返回值与 RN 不一致 |
| `⚠️ 需特定条件` | 需要特殊权限 / 需要设备硬件支持 |
| `❌ 不支持` | 鸿蒙 API 不存在 / 功能不可用 / 设备测试 fail |

**常见不支持场景**：
- 鸿蒙 API 返回 `null` 表示功能不可用
- API 需要 Telephony 硬件但设备未插卡
- API 需要系统级权限但应用无法获取
...
8. **使用示例**：从 `04-device-verify.json` 的 `test_scenarios`（或旧版 `04-testing.json`）提取关键场景描述，或从 Example 的 `App.tsx` 摘取核心调用片段

**7.2 写入 `05-summary.json`**
...
- **不修改 `ohos/README.md`**：占位符与 API 表由 **example生成**（`primary-04-example-gen` / `ohos-testing-guide`）在 HAP 成功后填写；本阶段只读其内容用于评分与报告，勿重写
- **不做代码级检查**：TurboModule 完整性、类型一致性、行为对等性等检查已由 testing 阶段的分析和设备验证覆盖，本阶段只聚合其结果
- **不修改 status.json**：status.json 由其他机制管理，本阶段不碰
- **容错处理**：example 产物优先 `04-testing.json`（兼容 `04-example.json` / `04-coding-example.json`）；设备/静态字段优先 `04-device-verify.json`（兼容旧版合并在 `04-testing.json` 的同名字段）。缺失时用保守默认值（`example_status`/`device_test_summary.status` 设为 `unknown`），并在报告中标注影响
