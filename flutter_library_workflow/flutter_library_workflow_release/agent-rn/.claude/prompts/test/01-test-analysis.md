...
| 类型 | 文件 |
|------|------|
| **输入** | PRD 文档（`--prd-path`，默认 `.rn-ohos-adaptation/01-analysis-prd.md`） |
...
3. 识别插件类型（platform_interaction/business_feature/ui_component/architecture_tool/ffi）
   > **插件类型识别**：Read `.claude/skills/01-test-analysis/references/plugin-types.md`（特征关键词、典型示例、测试策略映射）
...
> **完整示例**：Read `.claude/skills/01-test-analysis/references/ibo-model.md`（Input/Behavior/Output 表格 + Toast 示例）。
> **可视化输出**：Read `.claude/skills/01-test-analysis/references/analysis-visualization-guide.md`（状态迁移图、判定表、边界值表）。
...
1. Read `.claude/skills/01-test-analysis/assets/fixed-sections.md` → 固定章节模板
...
> **验证脚本**：`python .claude/skills/01-test-analysis/scripts/verify_report.py .rn-ohos-adaptation/01-test-analysis-report.md`
