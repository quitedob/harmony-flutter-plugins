...
| 文件 | 说明 |
|------|------|
| `.rn-ohos-adaptation/01-test-analysis-report.md` | 测试分析报告（或修订版） |
| `.rn-ohos-adaptation/01-test-points.json` | 测试点汇总 JSON |
...
2. 读取 `.claude/skills/02-test-case-gen/assets/test-cases-template.json` → 了解 JSON 格式模板
...
对每个功能模块：
1. Read `.claude/skills/02-test-case-gen/references/blackbox-test-guide.md` → 黑盒测试用例编写详细指南
2. 将每个测试点转化为一条测试用例，遵守红线原则 1-9
...
```
python .claude/skills/03-case-review/scripts/verify_test_cases.py .rn-ohos-adaptation
```
...
### 步骤 6：最终质量自检（本步骤为生成阶段的红线合规检查，评审打分由 case-review SubAgent 执行）
