...
function main() {
  ...
  const codingExample =
    loadJSON(path.join(adaptDir, "04-testing.json"))
    || loadJSON(path.join(adaptDir, "04-example.json"))
    || loadJSON(path.join(adaptDir, "04-coding-example.json"));
  const deviceVerify = loadJSON(path.join(adaptDir, "04-device-verify.json"));
  const summary = loadJSON(path.join(adaptDir, "05-summary.json"));

  /** 04 设备/静态字段：优先 device-verify 产物，兼容旧版合并在 04-testing 的库 */
  const deviceStage = deviceVerify || codingExample;
  ...
  if (deviceStage && summary && summary.runtime_check_summary) {
    const rtChecks = deviceStage.runtime_checks || [];
  ...
  // Check 7: device_test_summary consistency between 04-device-verify and 05
  if (deviceStage && summary && summary.device_test_summary) {
    const dtStatus = deviceStage.device_test_status;
    ...
    if (dtStatus && dtStatus !== "skipped" && deviceStage.device_test_results) {
      const dtResults = deviceStage.device_test_results || [];
  ...
  } else if (!deviceStage && summary && summary.device_test_summary) {
    if (
  ...
  if (summary && codingLib) {
    ...
    // Special case: total = 0 (pure JS library) means 100% coverage
    const coverageRate = total > 0 ? impl / total : 1.0;
    const buildPass = codingLib.build_status === "pass";
    const score = summary.quality_score;
    const isPureJS = total === 0;
    ...
    if (issues.length === 0) {
      const note = isPureJS ? " (纯 JS 库，无原生方法)" : "";
      addCheck(
        "quality_score_consistency",
        "pass",
        `评分 ${score} 与实际数据一致${note}`,
      );
    } else {
      addCheck(
        "quality_score_consistency",
