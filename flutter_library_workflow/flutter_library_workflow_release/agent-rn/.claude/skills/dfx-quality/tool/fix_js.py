...
用法:
    python fix_js.py --target <js_dir>
    python fix_js.py --target <js_dir> --dry-run
"""
...
def _extract_list_components(content: str) -> List[dict]:
    results = []
    for m in re.finditer(r'<(FlatList|SectionList)\b', content):
        tag_name = m.group(1)
        tag_start = m.start()
        ...
        while pos < len(content):
            ...
            if content[pos] == '>':
                ...
                break
            pos += 1
        tag_text = content[tag_start:pos]
...
def detect_flatlist_config(content: str, filename: str) -> List[str]:
    ...
    for comp in lists:
        ...
        if missing:
            issues.append(
                f"[1] WARNING: {comp['name']} [行 {comp['line']}] 缺性能配置: "
...
RE_RENDER_ITEM_COMPONENT = re.compile(
    r'renderItem\s*=\s*\{(?:\s*\([^)]*\)\s*=>\s*<\s*(\w+)|\s*(\w+))'
)
RE_MEMO_WRAP = re.compile(r'React\.memo\s*\(\s*(\w+)')
RE_MEMO_ASSIGN = re.compile(r'const\s+(\w+)\s*=\s*React\.memo')


def detect_missing_memo(content: str, filename: str) -> List[str]:
    issues = []
    has_list = re.search(r'<(FlatList|SectionList)\b', content)
    if not has_list:
        return issues

    item_components: Set[str] = set()
    for m in RE_RENDER_ITEM_COMPONENT.finditer(content):
    ...
    for m in RE_MEMO_WRAP.finditer(content):
        memo_components.add(m.group(1))
    for m in RE_MEMO_ASSIGN.finditer(content):
        memo_components.add(m.group(1))

    unmemoized = item_components - memo_components
    if unmemoized:
        issues.append(
            f"[2] WARNING: renderItem 引用的组件 {', '.join(sorted(unmemoized))} "
