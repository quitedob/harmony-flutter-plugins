...
拷贝 example 模板到 ohos/example/（ohos 已在短路径，无需单独处理 junction）。
"""
...
def templates_example_dir() -> str:
    return os.path.join(_SKILL_ROOT, "templates", "example")
...
def _supplement_example_files(src_dir: str, dst_dir: str) -> None:
    """增量补充 example：除 node_modules/oh_modules 仅补目录外，其余逐文件检查。"""
    from lib.incremental_copy import copy_tree_incremental, ensure_dependency_dirs_under

    n = copy_tree_incremental(src_dir, dst_dir, dry_run=False, log=print)
    n += ensure_dependency_dirs_under(dst_dir, dry_run=False, log=print)
    if n == 0:
        print("  所有模板文件已存在，无需补充")
    else:
        print(f"  共补充 {n} 项")
...
def main() -> None:
    ...
    p.add_argument("--skip-template", action="store_true", help="Skip template copy, only supplement missing files")
    p.add_argument("--light", action="store_true", help="Light copy: skip node_modules, oh_modules etc")
    p.add_argument("--dry-run", action="store_true")
    args = p.parse_args()

    root = os.path.abspath(args.plugin_root)
    dst = os.path.join(root, "ohos", "example")
    src = templates_example_dir()
    ...
    if args.skip_template:
        print(f"  dst: {dst}（跳过模板拷贝，补充缺失文件）")
        if not os.path.isdir(dst_real):
            print(f"  [error] example 目录不存在: {dst_real}")
            return
    else:
        print(f"  src: {src}")
        print(f"  dst: {dst}")
    ...
    if args.dry_run:
        print(f"  [dry-run] 将拷贝 example 模板并更新 package.json")
        return
    ...
    if not args.skip_template:
        ...
        print("  补充缺失文件...")
        _supplement_example_files(src, target_dir)
