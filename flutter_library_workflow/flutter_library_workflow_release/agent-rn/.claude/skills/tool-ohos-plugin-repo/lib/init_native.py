...
def _chain_ohos_codegen() -> None:
    ...
    if _MODULE_KIND in ("turbo", "both"):
        # 跳过 step_02（rn.py create 已拷贝模板）
        _run_python(gen_turbo, ["--plugin-root", _PLUGIN_ROOT, "1", "3", "4", "5", "6", "7"], cwd=_PLUGIN_ROOT)
    if _MODULE_KIND in ("fabric", "both"):
        # 跳过 step_02（rn.py create 已拷贝模板）
        _run_python(gen_fabric, ["--plugin-root", _PLUGIN_ROOT, "1", "3", "4", "5", "6", "7"], cwd=_PLUGIN_ROOT)
