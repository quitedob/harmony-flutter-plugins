...
用法：
    python generate_example.py [--plugin-root <path>] [--adaptation-dir <path>]

monorepo 仓库下 ohos/ 在 packages/<pkg>/，.ohos-adaptation/ 在仓库根，二者分离，
此时必须同时传 --plugin-root 与 --adaptation-dir。
"""
...
def _ensure_short_path_junction(plugin_root: Path, example_auto_dir: Path) -> Path | None:
    """长路径项目自动创建短路径 junction，规避 hvigor pnpm 索引超 Windows MAX_PATH。

    根因：hvigor 内部 pnpm 用 rnoh-hvigor-plugin-X.tgz 的全路径拼成 cache key 文件名，
    源路径 > 130 字符时，叠加 hash 后索引文件名 > 260 → Node.js fs 创建失败 →
    ERR_PNPM_ENOENT。详见 lessons/build.json: pnpm-cache-path-259。

    本函数检测 tgz 路径长度，超阈值时创建 `D:\\<short>` junction → plugin_root，
    后续 hvigor 编译从 junction 路径跑即可。短路径名取 plugin name 去符号后截断 8 字符。

    Returns:
        junction Path（创建成功）/ None（不需要或失败，主流程继续）
    """
    import sys
    if sys.platform != 'win32':
        return None

    # 1. 定位 rnoh-hvigor-plugin tgz（_ensure_node_modules 后才存在）
    tgz_candidates = list(
        (example_auto_dir / "node_modules" / "@react-native-oh" / "react-native-harmony-cli" / "harmony")
        .glob("rnoh-hvigor-plugin-*.tgz")
    )
    if not tgz_candidates:
        return None  # 没装到 hvigor plugin，无需 junction
    tgz_path = tgz_candidates[0]

    # 2. 路径长度阈值：tgz 全路径 > 130 字符才考虑（pnpm 加 hash+prefix 后会接近 260）
    THRESHOLD = 130
    if len(str(tgz_path)) <= THRESHOLD:
        return None

    # 3. 推短路径名：plugin name 去掉符号、转小写、截断 8 字符
    raw = re.sub(r'[^a-zA-Z0-9]', '', plugin_root.name).lower()[:8]
    if not raw:
        raw = "plugin"
    junction_root = Path(f"D:\\{raw}")

    # 4. 已存在且指向同一目录（junction 复用）→ 复用；否则跳过（不强行覆盖用户文件）
    if junction_root.exists():
        try:
            if os.path.samefile(junction_root, plugin_root):
                print(f"[INFO] 短路径 junction 已存在: {junction_root}")
                return junction_root
        except OSError:
            pass
        print(f"[WARN] D:\\{raw} 已存在且不是指向当前项目，跳过短路径创建")
        return None

    # 5. 创建 junction：mklink /J（不需要管理员权限）
    import subprocess
    try:
        subprocess.run(
            ['cmd', '/c', 'mklink', '/J', str(junction_root), str(plugin_root)],
            check=True,
            capture_output=True,
            text=True,
            encoding='utf-8',
        ...
        print(f"[WARN] 短路径 junction 创建失败: {e}")
        return None

    # 6. 验证 junction 可访问 example_auto
    short_tgz = junction_root / tgz_path.relative_to(plugin_root)
    if not short_tgz.is_file():
        print(f"[WARN] junction 创建成功但访问 tgz 失败: {short_tgz}")
        return None

    print(f"[OK] 短路径 junction 已创建: {junction_root} -> {plugin_root}")
    print(f"     rnoh-hvigor-plugin tgz 路径长度: {len(str(tgz_path))} -> {len(str(short_tgz))} 字符")
    print(f"     ⚠️ 后续 hvigor 编译必须从 junction 路径跑（避免 ERR_PNPM_ENOENT）：")
    print(f"        cd {junction_root}")
    print(f"        python .claude/skills/rn-ohos-template/rnohos.py build hap --example-dir example_auto")
    return junction_root
...
def main():
    ...
    parser.add_argument("--plugin-root", default=os.getcwd(), help="插件仓库根目录（ohos/ 所在）")
    parser.add_argument(
        "--adaptation-dir",
        default=None,
        help=".ohos-adaptation 目录。monorepo 下与 plugin-root 不同；不传则用 plugin-root/.ohos-adaptation",
    )
    args = parser.parse_args()

    plugin_root = Path(args.plugin_root)
    adaptation_dir = Path(args.adaptation_dir) if args.adaptation_dir else plugin_root / ".ohos-adaptation"
    ...
    print(f"[OK] plugin-root: {plugin_root}")
    print(f"[OK] adaptation-dir: {adaptation_dir}")
    ...
    # 长路径项目自动创建短路径 junction，规避 hvigor pnpm 索引超 Windows MAX_PATH
    # （仅在 Windows + tgz 路径 > 130 字符时触发；详见 lessons/build.json: pnpm-cache-path-259）
    _ensure_short_path_junction(plugin_root, example_auto_dir)
