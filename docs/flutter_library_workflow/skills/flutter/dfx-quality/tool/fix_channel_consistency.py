...
# -*- coding: utf-8 -*-
"""
fix_channel_consistency.py — Flutter DFX 跨层一致性检测
...
用法:
    python fix_channel_consistency.py --dart-target <lib_dir> --ets-target <ets_dir>
"""
...
# Channel 名称提取（ETS 端使用双引号）
RE_ETS_CHANNEL = re.compile(
...
def extract_dart_channels(filepath: str) -> Dict[str, Set[str]]:
    """从 Dart 文件提取 Channel 名称和 viewType，返回 {file: {name, ...}}"""
...
def extract_ets_channels(filepath: str) -> Dict[str, Set[str]]:
    """从 ETS 文件提取 Channel 名称和 viewType，返回 {file: {name, ...}}"""
    result: Dict[str, Set[str]] = {}
    with open(filepath, 'r', encoding='utf-8') as f:
        content = f.read()

    names: Set[str] = set()
    for m in RE_ETS_CHANNEL.finditer(content):
        names.add(m.group(1))
...
def check_consistency(
    ...
    for name in sorted(all_dart_names - all_ets_names):
        ...
        if name.startswith('[viewType]'):
            ...
            issues.append(
                f"[1] WARNING: Dart 端定义了 viewType '{view_type}' "
                f"(在 {', '.join(dart_files)}), "
                f"但 ETS 端未找到对应的 registerViewFactory — 两端 viewType 不一致"
            )
        else:
            issues.append(
                f"[2] WARNING: Dart 端使用了 Channel '{name}' "
                f"(在 {', '.join(dart_files)}), "
    ...
    for name in sorted(all_ets_names - all_dart_names):
        ...
        issues.append(
            f"[2] WARNING: ETS 端使用了 Channel '{name}' "
            f"(在 {', '.join(ets_files)}), "
...
def main():
    ...
    dart_files = list(dart_target.rglob('*.dart'))
    ets_files = list(ets_target.rglob('*.ets'))
    ...
    for fpath in sorted(dart_files):
        try:
            dart_map.update(extract_dart_channels(str(fpath)))
    ...
    for fpath in sorted(ets_files):
        try:
            ets_map.update(extract_ets_channels(str(fpath)))
        except Exception as e:
            if not args.json:
                print(f"ERROR: 读取 {fpath} 失败 — {e}")
