 ...
 *
 * adaptation_dir: absolute path to .ohos-adaptation/ directory
 */
...
function main() {
  ...
  // Check 1: Channel names in 03 must exist in 01
  if (analysis && codingLib) {
    const analysisChannelNames = new Set(
      (analysis.channels || []).map((c) => c.name),
    );
    const libChannels = new Set(
      (codingLib.implemented_methods || [])
        .concat(codingLib.not_implemented || [])
        .map((m) => m.channel),
    );
    const unknownChannels = [...libChannels].filter(
      (c) => !analysisChannelNames.has(c),
    );
    if (unknownChannels.length === 0) {
      addCheck(
        "channel_name_consistency",
        "pass",
        "03 中所有 channel 名称都在 01 的 channels 列表中",
      );
    } else {
      addCheck(
        "channel_name_consistency",
        "fail",
        `03 中引用了 01 未定义的 channel: ${unknownChannels.join(", ")}`,
      );
    }
  } else {
    addCheck(
  ...
  // Check 2: plugin_type_skill consistency between 02 and 03
  if (planning && codingLib) {
    const planSkill = planning.plugin_type_skill;
    const libSkill = codingLib.plugin_type_skill;
    if (planSkill === libSkill) {
      addCheck(
        "plugin_type_skill_consistency",
        "pass",
        `02 和 03 使用相同的 skill: ${planSkill}`,
      );
    } else {
      addCheck(
        "plugin_type_skill_consistency",
        "warning",
        `02 规划 skill "${planSkill}" 与 03 实际使用 "${libSkill}" 不一致`,
      );
    }
  } else {
    addCheck(
  ...
  if (summary && codingLib) {
    ...
    const coverageRate = total > 0 ? impl / total : 1;
