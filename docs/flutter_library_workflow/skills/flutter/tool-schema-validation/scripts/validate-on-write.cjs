...
function formatRenderResult(result) {
  ...
  return `❌ 报告生成失败: ${parsed.error || '未知错误'}`;
}
...
async function main() {
  ...
  if (!filePath.includes(".ohos-adaptation") && basename !== "plugins.json") {
    process.exit(0);
  }
  ...
  // 校验通过后自动生成 Markdown 报告
  let jsonParsed;
  try { jsonParsed = JSON.parse(jsonResult); } catch { jsonParsed = {}; }
  if (jsonParsed.valid && schemaKey !== "plugins") {
    const renderWorker = path.join(scriptsDir, "render-report-worker.cjs");
    if (fs.existsSync(renderWorker)) {
      console.log("\n📝 自动生成报告...");
      const renderResult = runWorker(renderWorker, [absoluteFilePath, schemaKey]);
      console.log(formatRenderResult(renderResult));
    }
  }
