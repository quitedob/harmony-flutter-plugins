...
* 开启混淆开关
  ...
  [build-profile.json5](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/master/ArkTS/ArkTSCompilationToolchain/ArkGuardForBytecodeObfuscation/ArkGuardBytecodeObfuscation/entry/build-profile.json5#L34-L47)
...
* consumer-rules.txt
  ...
  [build-profile.json5](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/master/ArkTS/ArkTSCompilationToolchain/ArkGuardForBytecodeObfuscation/ArkGuardBytecodeObfuscation/entry/build-profile.json5#L35-L48)
* obfuscation.txt
...
2. 待上述选项开启成功后，开启-enable-property-obfuscation
   ...
   1. 若代码中存在静态定义、动态访问的情况或者动态定义、静态访问的情况，需要使用-keep-property-name保留属性名称。示例：
      ...
      [BytecodeObfuscation.ts](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/master/ArkTS/ArkTSCompilationToolchain/ArkGuardForBytecodeObfuscation/ArkGuardBytecodeObfuscation/entry/src/main/ets/bytecodeobfuscation/BytecodeObfuscation.ts#L233-L241)
      ...
      [BytecodeObfuscation.ts](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/master/ArkTS/ArkTSCompilationToolchain/ArkGuardForBytecodeObfuscation/ArkGuardBytecodeObfuscation/entry/src/main/ets/bytecodeobfuscation/BytecodeObfuscation.ts#L243-L250)
   ...
   4. 验证应用功能，排查遗漏的场景。若应用出现功能异常，可依据混淆后的报错栈，在模块的 **build/default/[...]/release/obfuscation/** 目录下查阅 **nameCache.json**（名称映射表）、**config.json**（混淆项与白名单）等产物，按[查看混淆效果](/consumer/cn/doc/harmonyos-guides/bytecode-obfuscation-guide#查看混淆效果)对照定位源码行并排查白名单。确定需保留的名称后，使用-keep-property-name进行保留。
3. 待上述选项应用适配成功后，开启-enable-export-obfuscation选项。此选项开启后以下场景需要适配：
   ...
   4. 验证应用功能以及模块被依赖时的接口调用功能，排查遗漏的场景。若应用出现功能异常，可依据混淆后的报错栈，在模块的 **build/default/[...]/release/obfuscation/** 目录下查阅 **nameCache.json**（名称映射表）、**config.json**（混淆项与白名单）等产物，按[查看混淆效果](/consumer/cn/doc/harmonyos-guides/bytecode-obfuscation-guide#查看混淆效果)对照定位源码行；并按需使用-keep-global-name、-keep-property-name等进行保留。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/45/v3/ITY47F9qTk-HZKmu5un40w/zh-cn_image_0000002596776097.png?HW-CC-KV=V1&HW-CC-Date=20260507T114732Z&HW-CC-Expire=86400&HW-CC-Sign=FB079A474229B6AF2E9CB1FCA5E89B569382BB276D3B70CDA7E61B276CEE4B3D)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/82/v3/YDkPijVOSDyvgia68cHDuA/zh-cn_image_0000002566336418.png?HW-CC-KV=V1&HW-CC-Date=20260507T114732Z&HW-CC-Expire=86400&HW-CC-Sign=264413E5884B3CE7C417105E6D745035E6D0CEBA8CCE130A2BC9EE77741F4747)