...
5. 对于本地源码HAR包和本地HSP包，ArkGuard不会自动将其**模块源码目录下各 .ets、.ts 源文件**中的export导出名称收集到不混淆名单中。此类依赖属于工程内本地模块（**非发布态**、未安装至工程级 oh\_modules），**不参与**与远程发布包相同的跨模块依赖解析及export自动收集流程，因此需由开发者按需配置白名单或借助混淆助手生成保留规则。
   ...
   | --- | --- | --- | --- | --- |
   | [本地源码HAR包](/consumer/cn/doc/harmonyos-guides/source-obfuscation-practice#本地源码har包) | 模块在本地的源码目录 | 是 | 是 | 否 |
   | [发布态源码HAR包](/consumer/cn/doc/harmonyos-guides/source-obfuscation-practice#发布态源码har包) | 工程级的oh\_modules下的模块目录 | 是 | 是 | 是 |
   | [发布态字节码HAR包](/consumer/cn/doc/harmonyos-guides/source-obfuscation-practice#发布态字节码har包) | 二进制字节码和声明文件 | 否 | 是 | 是 |
   | [本地源码HSP包](/consumer/cn/doc/harmonyos-guides/source-obfuscation-practice#本地源码hsp包) | NA | 否 | 是 | 否 |
   | [集成态HSP包](/consumer/cn/doc/harmonyos-guides/source-obfuscation-practice#集成态hsp包) | 二进制字节码和声明文件 | 否 | 是 | 是 |
