  ...
  "ohos_api_mapping": [
    {
      ...
      "notes": "补充说明（可选）",
      "mapping_type": "direct | architectural | null（architectural=步骤3.5架构级异构替代）"
    }
  ],
...
| `source` | 是 | 数据来源 |
| `notes` | 否 | 补充说明 |
| `mapping_type` | 否 | 映射类型：`direct`=步骤3 API级1:1映射（默认），`architectural`=步骤3.5架构级异构替代，`null`=未指定 |
...
| 功能 | TurboModule 方法 | 类型 | 鸿蒙模块 | 鸿蒙 API | 置信度 | 来源 |
|------|-----------------|------|----------|----------|--------|------|
| ...  | ...             | direct/architectural | ...      | ...      | ...    | ...  |

> **映射类型说明**：`direct`=API级1:1映射，`architectural`=架构级异构替代

### 架构级方案（如有）

> 仅当 `mapping_type = "architectural"` 的记录存在时显示此章节。
