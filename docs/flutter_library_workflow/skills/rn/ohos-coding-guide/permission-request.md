...
## 脚手架自动注入（`rn.py create harmony`）

若仓库中已有 `.rn-ohos-adaptation/02-planning.json`，执行 `rn.py create harmony` 时会根据 `permission_mapping` 自动写入：

| 文件 | 内容 |
|------|------|
| `harmony/{short}/src/main/module.json5` | `requestPermissions`（含 user_grant 的 `reason`） |
| `harmony/{short}/src/main/resources/base/element/string.json` | 各 `reason` 对应的 `$string:` 文案 |
| `harmony/{short}/src/main/ets/PermissionHelper.ets` | `checkAccessToken` + `requestPermissionsFromUser` 封装 |
| `harmony/{short}/PERMISSIONS.md` | 列出 system_grant / user_grant 及说明 |

**Agent 仍需**：在调用相关 API 的 TurboModule 方法内调用 `ensureUserGrantPermissions()`（或等价逻辑）；`check_lib_static.py` 会校验 user_grant 权限是否在 ETS/TS 中出现动态申请。
...
> 若 `reason` 使用 `$string:xxx` 引用，须在 `resources/base/element/string.json` 中定义对应资源，否则 `build har` 编译失败。
...
## 常见误判（禁止照搬 planning 而不查 API 文档）

| 权限 | 实际用途 | 误判场景 |
|------|----------|----------|
| `ohos.permission.SYSTEM_FLOAT_WINDOW` | 创建**悬浮窗** overlay（受限开放，`system_grant`） | 误用于 `@ohos.window` 的 `setSpecificSystemBarEnabled`、`setWindowSystemBarProperties`、`setWindowLayoutFullScreen` 等**本应用主窗口**系统栏控制——这些 API **无「需要权限」字段**，**不要**声明此权限 |

误声明 `SYSTEM_FLOAT_WINDOW` 会导致 Example HAP 安装失败（`install failed due to grant request permissions failed`，code `9568289`），`onDeviceTest` 在 `GenerateDeviceCoverage` 阶段卡住或报错。