...
```bash
python ".claude/skills/rn-plugin-example-generator/tool/generate_example.py"
```

生成器会：
1. **拷贝 ohos/example 目录到 ohos/example_auto**（如果 example 目录存在）
2. **清理不需要的文件**（包括 node_modules、.harmony 等自动生成的文件）
3. 读取 `.rn-ohos-adaptation/04-test-cases.json`
4. 输出完整的代码结构到 `ohos/example_auto`：
...
1. **参考资料**：
   - 打开 `.rn-ohos-adaptation/01-analysis-prd.md` 的公开 API 规格
   - 参考 `.rn-ohos-adaptation/04-test-cases.json` 中的测试用例信息
...
> 
> 若发现脚本误报或遗漏，记录到 `.rn-ohos-adaptation/check-script-issues.md` 供后续优化。
...
# 编译 HAP（将 bundle.harmony.js 打包进 HAP）
cd ohos/example_auto/harmony
...
- 测试用例文件：`.rn-ohos-adaptation/04-test-cases.json`