...
- 被修复的鸿蒙库根目录 **`ohos/`**（含 `oh-package.json5`、`Index.ets`/入口、`module.json5`、`build-profile.json5`）——唯一的修复对象

> 修复范围严格限定在插件的 `ohos/` 鸿蒙库，**不修改** `example/`（含 `example/ohos/`，Demo）。
...
- 允许的修复类型：补充错误处理、修正资源泄漏、补充缺失的类型标注、修正命名规范、调整依赖方向、CodeLinter 报告的代码规范问题、DFX 检测到的不涉及功能逻辑的稳定性/性能/功耗/UX/兼容性问题。
...
修复后重新构建验证，编译失败必须根据日志修复真实问题后重试，**不得**用删功能、空实现、写死可配置项换取构建成功。

编译方式参考 CLAUDE.md「编译与验证」章节，使用 `flutter build hap` 命令，不要自行探索 hvigor 构建链。
