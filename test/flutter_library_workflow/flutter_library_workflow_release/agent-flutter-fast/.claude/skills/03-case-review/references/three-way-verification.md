...
5. 用例 ID 校验：测试点 ID、测试用例 JSON ID、测试用例 Markdown ID 完全一致，保持 `F-XX-YY` 格式

**使用方式**：

```bash
# 命令行执行
python scripts/verify_test_cases.py 02-test-points.json 04-test-cases.json 04-test-cases.md
```

**Python 调用**：

```python
from scripts.verify_test_cases import verify_test_cases_three_way, verify_json_schema

# 三方一致性验证
passed, results = verify_test_cases_three_way(
    '02-test-points.json',
    '04-test-cases.json',
    '04-test-cases.md'
)
