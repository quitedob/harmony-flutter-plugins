...
`case-review` Skill 专门用于评审测试用例（阶段 2 产物）的质量和规范性。

**适用范围**：
- 测试用例评审（阶段 3）
- 评审对象：`02-test-cases.md`、`04-test-cases.json`、`01-test-points.json`
...
| `input_files` | 是 | 输入文件路径数组：`[02-test-cases.md, 04-test-cases.json, 01-test-points.json, 01-test-analysis-report.md]` |
...
| `{output_dir}/02-test-cases-revised.md` | 修订后的测试用例（如有修改） |
...
- [ ] 用例 ID 符合规范，并与测试点 ID 完全一致（如 F-01-01）
- [ ] Markdown 表格格式正确
- [ ] JSON 格式有效
...
**评审报告表格格式**：
- [ ] 三方比对表：使用 `| 来源 | 用例总数 |` 或 `| 来源 | L0 | L1 | L2 |` 表头
