...
1. 判断当前系统是否支持向量数据库，若不支持，则表示当前系统不具备向量数据库能力。示例代码如下：
   ...
   3. // ...
   4. // 判断当前系统是否支持向量数据库
   5. let ret = relationalStore.isVectorSupported();
   6. if (!ret) {
   7. console.error(`vectorDB is not supported.`);
   8. return;
   9. }
   ```

   [vectorStoreCTUD.ets](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20251117/ArkData/VectorStore/entry/src/main/ets/pages/crud/vectorStoreCTUD.ets#L16-L42)
2. 若支持向量数据库则需要获取一个RdbStore。通过getRdbStore接口创建数据库，并执行建表操作。
   ...
   * 错误码的详细介绍请参见[通用错误码](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/errorcode-universal)和[关系型数据库错误码](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/errorcode-data-rdb)。
   ...
   2. let context = getContext();
   3. const STORE_CONFIG: relationalStore.StoreConfig = {
   4. name: 'VectorTest.db', // 数据库文件名
   5. securityLevel: relationalStore.SecurityLevel.S1, // 数据库安全级别
   6. vector: true // 可选参数，该参数为true时才可以使用向量数据库。
   7. };
   8. // ...
   9. try {
   10. store = await relationalStore.getRdbStore(context, STORE_CONFIG);
   11. // 建表语句，floatvector(2)代表repr的维度是2
   12. const SQL_CREATE_TABLE = 'CREATE TABLE IF NOT EXISTS test (id INTEGER PRIMARY KEY, repr floatvector(2));';
   13. // 第二个入参表示不开启显示事务，第三个参数undefined表示未使用参数绑定
   14. await store!.execute(SQL_CREATE_TABLE, 0, undefined);
   15. } catch(err) {
   16. console.error(`Get RdbStore failed, code is ${err.code}, message is ${err.message}`);
   17. };
   ```

   [vectorStoreCTUD.ets](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20251117/ArkData/VectorStore/entry/src/main/ets/pages/crud/vectorStoreCTUD.ets#L23-L54)
3. 获取到RdbStore后，调用execute接口插入数据。
   ...
   [vectorStoreCTUD.ets](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20251117/ArkData/VectorStore/entry/src/main/ets/pages/crud/vectorStoreCTUD.ets#L56-L66)
4. 获取到RdbStore后，调用execute接口修改或删除数据。示例代码如下：
   ...
   [vectorStoreCTUD.ets](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20251117/ArkData/VectorStore/entry/src/main/ets/pages/crud/vectorStoreCTUD.ets#L68-L89)
5. 获取到RdbStore后，调用querySql方法查找数据，返回一个ResultSet结果集。
   ...
   [vectorStoreCTUD.ets](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20251117/ArkData/VectorStore/entry/src/main/ets/pages/crud/vectorStoreCTUD.ets#L91-L139)
6. 创建视图并执行查询。示例代码如下：
   ...
   [vectorStoreCTUD.ets](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20251117/ArkData/VectorStore/entry/src/main/ets/pages/crud/vectorStoreCTUD.ets#L141-L151)
7. ‌使用向量索引进行查询，提升查询效率。
   ...
   * 向量索引的命中条件。必须是ORDER BY + LIMIT类型的查询，ORDER BY只有一个排序条件，这个条件是向量距离条件；ORDER BY与DESC连用，不会使用向量索引；查询距离度量与创建索引时的度量需要保持一致，例如创建向量索引时使用L2，在查询时使用<->进行度量才可以命中索引。
   ...
   [vectorStoreCTUD.ets](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20251117/ArkData/VectorStore/entry/src/main/ets/pages/crud/vectorStoreCTUD.ets#L153-L171)
8. 手动回收索引删除产生的磁盘碎片。从API version 20 开始支持此功能。
   ...
   * 磁盘碎片回收任务由后台基于负载自动调度，通常仅在低负载场景下执行，成功所需时间依赖负载情况。
   ...
   [vectorStoreCTUD.ets](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20251117/ArkData/VectorStore/entry/src/main/ets/pages/crud/vectorStoreCTUD.ets#L173-L180)
9. 配置数据老化功能。当应用的数据需要经常清理时，可以按时间或空间配置数据老化策略，从而实现数据的自动化清理。
   ...
   **表4** 数据老化策略参数(parameter)
   ...
   [vectorStoreCTUD.ets](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20251117/ArkData/VectorStore/entry/src/main/ets/pages/crud/vectorStoreCTUD.ets#L182-L189)
10. 配置数据压缩功能。该功能在建表时配置，可以压缩数据类型为text的列数据。
    ...
    [vectorStoreCTUD.ets](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20251117/ArkData/VectorStore/entry/src/main/ets/pages/crud/vectorStoreCTUD.ets#L191-L198)
11. 删除数据库。
    ...
    [vectorStoreCTUD.ets](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20251117/ArkData/VectorStore/entry/src/main/ets/pages/crud/vectorStoreCTUD.ets#L200-L208)