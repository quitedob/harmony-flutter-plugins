 ...
 * 2. 校验前置产物存在：.rn-ohos-adaptation/03-coding-library.json（testing 依赖 coding 输出）
...
function checkCodingPrerequisite() {
  ...
  const prod = path.join(repoPath, '.rn-ohos-adaptation', '03-coding-library.json');
  if (!fs.existsSync(prod)) {
    console.error(
      '鸿蒙测试门禁（testing/example-gen）：缺少前置产物 .rn-ohos-adaptation/03-coding-library.json。\n' +
