...
文档结构：**快速开始** → **环境门禁** → **文档检索 Skills** → **示例仓库验证** → **产物目录关系** → **第 5 节：适配路径概述** → **第 6 节：Windows 长路径** → **第 7 节：rn.py 工具使用**。

---

## 快速开始

按下列步骤可在本地跑通 **RN 插件 → 鸿蒙** 适配流水线（管理面板 + Agent）。更细的环境说明见仓库根目录 [README.md](../README.md)。

### 前置条件（简要）

| 项 | 要求 |
|----|------|
| **Node.js / npm** | Node ≥ 18，用于管理面板与脚本 |
| **Python 3** | ≥ 3.12，运行 `rn.py` 与门禁脚本（不支持 3.11 及以下） |
| **Git** | 克隆插件仓库 |
| **DevEco Studio** | 已安装；编码/测试阶段需其内置 `node` / `java` / `hvigorw`（非默认路径可设 `DEVECO_HOME`） |
| **OpenCode** | 已安装并可执行 `opencode`（如 `npm i -g opencode-ai`），且已配置大模型 API Key |
| **Harmony-Skills** | 见下方「步骤 1」 |

### 步骤 1：安装文档检索 Skills（一次性）

从 [**HarmonyOS-AI/Harmony-Skills**](https://github.com/HarmonyOS-AI/Harmony-Skills) 将 **`harmonyos-sdk-api-lookup`**、**`harmonyos-docs-lookup`** 两个目录整包拷贝到：

- **项目本地**：`agent-rn\.claude\skills\`
- **或全局目录**：`~/.agents/skills/`（推荐，多项目共享）

详见 [第 2 节](#2-安装文档检索-skills-harmony-skills)。

### 步骤 2：预初始化 rn.py 模板（一次性，推荐）

在任意模块适配前执行一次，可显著加快后续 `create` / `init`（编码阶段门禁未初始化时也会自动执行，但首次可能等待数分钟）：
...
详见 [7.3 `rn.py init-template`](#73-rnpy-init-template)。

### 步骤 3：配置 `opencode.json` 目录放行

OpenCode 默认不允许访问工作区外的目录。如果插件仓库或测试阶段的 Example 工程位于非默认路径，须在 **`agent-rn/opencode.json`** 的 `permission.external_directory` 中按**实际路径**添加放行项：

**Windows 示例**：
> 说明：`D:/code/f/**` 为仓库路径，按实际情况修改；`D:/rn/**` 为短路径目录（**目录名 `rn` 固定，不可修改**），用于存放 Example 工程（Windows 长路径限制的缓解方案，详见[第6节](#6-windows-长路径与当前对策)）。盘符 `D` 换成你本机实际盘符。
```json
"permission": {
  "external_directory": {
    "D:/code/f/**": "allow",
...
**macOS 示例**（无长路径问题，一般只需放行仓库路径）：
```json
"permission": {
  "external_directory": {
    "/Users/yourname/projects/**": "allow"
  }
}
```

将路径替换为你的实际仓库根目录和 Example 存放路径。Windows 上进入编码阶段时，门禁还会额外校验当前盘符下是否含 `…/rn/**` 放行项；详见 [1.2 节](#12-门禁脚本具体检查什么) 与 [第 6 节](#6-windows-长路径与当前对策)。

### 步骤 4：配置签名（可选，真机测试需要）

如需真机运行 HAP 验证，需配置签名。从模板文件拷贝并填入本机真实签名值：

```bash
cp adapt-workflow/data/signing.example.json adapt-workflow/data/signing.rn.local.json
```

编辑 `signing.rn.local.json`，填写 DevEco Studio 生成的签名配置（`certpath`、`profile`、`storeFile`、密钥等）。签名配置在 `rn.py build hap` 时会自动同步到 `build-profile.json5`。
...
浏览器打开 **http://localhost:3000**。模型档位与 API Key 见根目录 README「AI 大模型 API Key 配置」。

### 步骤 6：添加 RN 插件并克隆

1. 在面板顶部选择 **React Native → 转鸿蒙** Profile。
2. 粘贴插件 Git 地址并添加，系统会克隆到 `repos-rn/` 下并在插件根目录挂上 `agent-rn` 工作区链接。

**试跑示例**（体量小、TurboModule）：

`https://github.com/wneel/react-native-get-device-locale.git`

### 步骤 7：按阶段执行适配

进入插件详情页，依次执行 **分析 → 规划 → 编码（库）→ 测试（Example + HAP）→ 总结**（或使用「全部执行」）。进入 **编码** 时会运行 [门禁脚本](#12-windows门禁脚本具体检查什么)；失败时按 stderr 提示配置 DevEco 或调整 `agent-rn/opencode.json` 中的 `permission.external_directory`（Windows 短路径 `D:\rn\` 等场景见 [第 6 节](#6-windows-长路径与当前对策)）。

**可选预检**（在编码前手动跑门禁）：

```bash
node agent-rn/.scripts/gate-coding.js <插件仓库根目录绝对路径>
```
...
流水线跑通后，插件仓库根目录通常会出现 **`ohos/`**（含 `harmony/library`、`example`）与 **`.rn-ohos-adaptation/`**。在 **DevEco Studio** 中打开 **`ohos/example/harmony`**，运行到真机验证。

目录关系见 [第 4 节](#4-跑通后新增与关键目录的关系)；手动适配与 `rn.py` 命令见 [第 5、7 节](#5-适配路径概述从-js-契约到鸿蒙集成)。

---

## 1. 环境检查与门禁（避免运行期报错）

执行 **编码阶段**（`primary-03-coding-library`）前，管理面板将运行本目录门禁脚本（见 `opencode.json` 中该阶段的 `gate_script`）。门禁会检测 `tool-ohos-plugin-repo/templates` 是否已执行过 `rn.py init-template`，**未执行则自动运行一次**；建议在进入编码阶段前于本地完成预检，以降低 `build har` / 后续测试阶段构建失败概率。
...
门禁实现见 [`.scripts/gate-coding.js`](.scripts/gate-coding.js)。

| 检查项 | 平台 | 含义 |
|--------|------|------|
| **templates 已 init-template** | 全平台 | 检查 `templates/example/node_modules`、`example/harmony/oh_modules`、`har_wrapper/oh_modules`、`ohos/node_modules` 等；缺失则自动执行 `python tool/rn.py init-template`（可能耗时数分钟）。 |
| **DevEco 工具链可解析** | 全平台 | 在 DevEco Studio 安装目录下能找到内置 `node`、`java`、`hvigorw`（macOS 默认 `/Applications/DevEco-Studio.app`，由 `adapt-workflow/lib/deveco-toolchain.js` 扫描）。**无需**手动把 DevEco 工具加入系统 PATH；管理面板运行 Agent 时会自动注入。非默认路径可设 `devecoHome` 或 `DEVECO_HOME`。 |
| **`permission.external_directory` 放行本盘 `…/rn/**`** | 仅 Windows | 短路径 `D:\rn\…` 场景下，要求 `opencode.json` 含当前盘符的 `rn` 放行项（如 `D:/rn/**`）。macOS 仓库路径通常无需此项。 |
...
退出码 `0` 表示通过；失败时 **stderr** 会打印具体原因（DevEco 工具链、`external_directory` 等）。

### 1.3 macOS 说明

- **DevEco 工具链**：门禁会检查 `/Applications/DevEco-Studio.app`（或 `devecoHome` / `DEVECO_HOME`）内是否具备 `node` / `java` / `hvigorw`。
- **`external_directory` 放行**：如果插件仓库位于非默认路径，需在 `opencode.json` 中添加对应的 `allow` 规则：
  ```json
  "permission": {
    "external_directory": {
      "/Users/yourname/projects/**": "allow"
...
规划、编码、测试等阶段会通过 **`sub-doc-search`** 查询鸿蒙 **SDK API** 与**开发文档**。请从仓库 [**HarmonyOS-AI/Harmony-Skills**](https://github.com/HarmonyOS-AI/Harmony-Skills) 取得下列两个 Skill，**整目录拷贝**到以下位置之一：

- **项目本地**：`agent-rn\.claude\skills\`
- **全局目录（推荐）**：`~/.agents/skills/`，多项目共享

- **`harmonyos-sdk-api-lookup`** — 本地检索 HarmonyOS SDK 的 `.d.ts` 等，用于 API 签名与类型。
- **`harmonyos-docs-lookup`** — 检索官方开发指南、权限、Kit 等文档。

拷贝后对应目录下应能直接看到上述目录名。若 Skills 仓库中目录命名有版本后缀，以仓库内实际文件夹名为准，保持 Skill 名称可被 `sub-doc-search` 路由识别即可。
...
3. 按顺序执行各阶段；进入 **编码** 时将触发门禁。失败时依据门禁输出安装/配置 DevEco（或 `devecoHome` / `DEVECO_HOME`），或调整 **`agent-rn/opencode.json`** 中 **`permission.external_directory`** 后重试。  
...
```
插件仓库根 (CWD)
├── package.json                  # 原始 JS 侧配置 (含 harmony 字段、autolinking 等)
├── src/                          # JS/TS 源码
│   ├── *Spec.ts / *Spec.tsx      # TurboModule / Fabric 的 Codegen Spec
│   ├── index.ts / *.ts(x)        # 对外导出、Platform.OS === 'harmony' 分支等
│   └── …                         # 视模块而定
├── android/                      # 原有 Android 原生实现
├── ios/                          # 原有 iOS 原生实现
├── ohos/                         # 鸿蒙适配工程目录 (由 rn.py create 生成)
│   ├── package.json              # ohos 工程的 JS 依赖配置
│   ├── src/                      # ohos 工程的 JS 源码
│   │   └── specs/                # Codegen Spec 目录 (v1/v2)
│   ├── harmony/
│   │   ├── library/              # 鸿蒙原生库模块 (ETS/C++、oh-package.json5、module.json5 等)
│   │   └── library.har           # HAR 包产物 (需提交到 git)
│   ├── example/                  # 验证用 RN + OHOS Example
│   │   ├── App.tsx               # RN 侧入口 UI：按已实现 API 生成调用/展示
│   │   ├── package.json          # Example RN 依赖配置
│   │   └── harmony/              # 鸿蒙套壳工程
│   │       ├── entry/            # HAP 入口模块 (ets/cpp/resources)
│   │       ├── oh-package.json5  # 鸿蒙依赖配置
│   │       └── build-profile.json5 # 构建配置
│   └── .rn-build/
│       └── har_wrapper/          # HAR 构建包装目录 (由 rn.py build har 使用)
...
2. **OpenCode 目录权限**：**`agent-rn/opencode.json`** 中 **`permission.external_directory`** 对 **`D:/rn/**`** 等放行，避免 CLI 拒绝访问上述短路径工作区（与 **第 1 节** 门禁一致）。  
...
| 命令 | 说明 |
|------|------|
| `rn.py create --template=plugin [--force] [--rn-version=0.72\|0.77\|0.82]` | 生成 ohos/ 脚手架；`--rn-version` 指定目标 RN 版本（默认 0.72） |
...
**`rn.py init-template` 会预装 templates 依赖，显著加快后续 `create`/`init`。** 可手动执行一次；进入 **coding-library** 时门禁也会在未初始化时**自动执行**。
...
### 7.5 更新已适配库数据（rn-adapted-library）

`rn-adapted-library` Skill 维护了 RN 三方库的鸿蒙适配状态数据库，数据来源为 [OpenHarmony-RN/usage-docs](https://gitcode.com/OpenHarmony-RN/usage-docs)。当上游数据更新时，执行以下两步即可同步：

```bash
# 1. 拉取最新 usage-docs（首次会 clone，后续 pull 即可）
cd agent-rn/.claude/skills/rn-adapted-library/_usage-docs/usage-docs && git pull

# 2. 执行同步脚本，自动生成 JSON + MD
node agent-rn/.claude/skills/rn-adapted-library/scripts/sync-adapted-libraries.js
```

脚本会：
- 解析 `usage-docs/zh-cn/README.md` 中的 **RNOH 三方库总览** 和 **JS 三方库** 两张表
- 去重（同名库取 `adapted > in_development > js_only > not_adapted` 优先级最高者）
- 应用 `MANUAL_OVERRIDES`（补全 README 中信息不完整的条目，如缺失的 npm_url）
- 输出 `references/adapted-libraries.json` 和 `references/adapted-libraries.md`
- 打印 Diff 统计（新增 / 移除 / 状态变更 / 版本变更）

如需为特定条目补充 README 中缺失的信息，编辑脚本顶部的 `MANUAL_OVERRIDES` 对象即可。

#### 同步 dep-version-map（RN 版本 → OHOS 包版本映射）

```bash
# 确保 usage-docs 已拉取（同上）
cd agent-rn/.claude/skills/rn-adapted-library/_usage-docs/usage-docs && git pull
...
脚本会：
- 遍历 `usage-docs/zh-cn/*.md` 单库文档，解析版本兼容性表格
- 提取每个库在 RN 0.72 / 0.77 / 0.82 下的 OHOS 包名和版本号
- 检测 `dual_install`（需同时安装原始包的情况）
- 输出 `references/dep-version-map.json`，供 02-planning 阶段的 RN 版本选择和依赖版本解析使用

### 7.6 测试用例
| 类型 | 级别 | 仓库 |
|------|------|------|
|turbo|低|https://github.com/wneel/react-native-get-device-locale|
|turbo|中|https://github.com/dev-family/react-native-device-country|
|turbo|高|https://github.com/callstack/react-native-image-editor|
|fabric|高|https://github.com/fbeccaceci/react-native-fast-squircle|
|js-only|低|https://github.com/gameboyVito/react-native-ultimate-listview|
|js-only|极低|https://github.com/xgfe/react-native-datepicker|
|老转新turbo|低|https://github.com/tessus/react-native-version-info|
|老转新turbo|低|https://github.com/AlexanderZaytsev/react-native-i18n|
|老转新turbo|中|https://github.com/kimxogus/react-native-version-check|
|老转新turbo|高|https://github.com/PeelTechnologies/react-native-tcp|
