...
def verify_test_cases_three_way(test_points_path: str, test_cases_json_path: str, test_cases_md_path: str) -> Tuple[bool, Dict]:
    ...
    tc_md_modules = len(re.findall(r'^### (F-\d+):', test_cases_md, re.MULTILINE))
    ...
    tc_md_total = len(re.findall(r'^\| (F\d+-\d+) \|', test_cases_md, re.MULTILINE))
    ...
    md_levels = {}
    for match in re.finditer(r'^\| (F\d+-\d+) \|.*\| (L[0-3]) \|', test_cases_md, re.MULTILINE):
        level = match.group(2)
