...
| **@State 行内初始化引用后声明字段** | `@State config = this.buildConfig()` 中 `buildConfig` 读取了声明在 `config` 后的 `COLORS`/`@State` 字段 → 运行时 crash（`Cannot read property 'length/get' of undefined`）。修复：`@State` 行内只给默认值 `new Config()`，真实初始化延迟到 `aboutToAppear()`。详见 `ohos-coding-guide/arkui-state-reference.md` Pitfall #14。 |
