...
| SubAgent | 调用时机 | 输入 | 输出 |
|----------|----------|------|------|
| sdk-test-analysis | 阶段 1 | PRD 文档 | 01-test-analysis-report.md, 01-test-points.json |
| sdk-test-case-gen | 阶段 2 | 测试分析报告 + 测试点 JSON | 04-test-cases.json |
| sdk-test-case-gen（修订） | 阶段 3 评审不通过时 | 测试分析报告 + 测试点 JSON + 评审报告 | 修订后的 04-test-cases.json |
| sdk-case-review | 阶段 3 | 测试用例 | 03-case-review-report.md |

**调用方式**：通过 Agent 工具委派机制调用对应 SubAgent。主 Agent 将对应 prompt 文件的完整路径作为 SubAgent 的 prompt 参数传递（例如：`../../agent-android-sdk/.claude/prompts/test/01-test-analysis.md`），SubAgent 自行读取 prompt 并按其中步骤执行。

**⚠️ SubAgent 委派方式（按运行平台）**：
- **OpenCode 平台**：使用 `task` 工具，参数为 subagent 名称（`sdk-test-analysis` / `sdk-test-case-gen` / `sdk-case-review`）
- **Claude Code 平台**：SubAgent 已通过 `--agents` JSON 内联注入，通过 claude 内置的 subagent/task 机制调用
- **通用方式**：如平台不支持特定委派工具，直接在 prompt 中指明读取 `../../agent-android-sdk/.claude/prompts/test/{对应prompt}.md`，按其中步骤执行
**参数传递映射**：
| 主 Agent 参数 | SubAgent 参数 | 说明 |
|--------------|--------------|------|
| `--prd-path` | 传给 sdk-test-analysis | PRD 文档路径 |
| `--output-dir` | 传给所有 SubAgent | 输出目录 |
| `--case-level` | `--level`（sdk-test-case-gen） | 用例生成级别（all/L0） |
| `--revision-report` | `--revision-report`（sdk-test-case-gen） | 评审修订报告路径（评审不通过时回传） |

---

## ⚠️ 路径规范（强制）

- 所有 SubAgent prompt 文件直接使用相对路径读取，**禁止使用 Glob 搜索**：
  - 阶段 1：`../../agent-android-sdk/.claude/prompts/test/01-sdk-test-analysis.md`
  - 阶段 2：`../../agent-android-sdk/.claude/prompts/test/02-sdk-test-case-gen.md`
  - 阶段 3：`../../agent-android-sdk/.claude/prompts/test/03-sdk-case-review.md`
- 所有输出目录统一使用 `.ohos-adaptation/` 前缀（注意开头的点号）
- 一致性校验脚本必须使用 `./.ohos-adaptation/` 路径（注意开头的点号）：
  ```
  node -e "const tp = require('./.ohos-adaptation/01-test-points.json'); const tc = require('./.ohos-adaptation/04-test-cases.json'); ..."
  ```
...
**执行步骤**：
1. 委派 SubAgent `sdk-test-analysis`，传入参数 `--prd-path` 和 `--output-dir`
2. **SubAgent 返回后，从返回消息中提取 ` ```json ` 代码块内的完整 JSON 内容，由主 Agent 使用 write 工具写入 `{output_dir}/01-test-points.json`**
3. 验证输出文件存在：
...
- `summary.totalPoints` → 测试点总数（阶段2、阶段3用于一致性校验的基准值）
...
2. 委派 SubAgent `sdk-test-case-gen`，传入参数 `--output-dir` 和 `--case-level`
3. **SubAgent 返回后，从返回消息中提取 ` ```json ` 代码块内的完整 JSON 内容，由主 Agent 使用 write 工具写入 `{output_dir}/04-test-cases.json`**
4. 验证输出文件存在：
...
6. **阶段间版本一致性验证**：
   - 比对阶段2生成的用例总数与阶段1提取的 `summary.totalPoints` 是否一致
   - 如不一致，输出警告并记录差异详情
...
**执行步骤**：
1. 委派 SubAgent `sdk-case-review`
2. 读取评审报告 Markdown
3. 检查评审结论：
   - **通过** → 流程完成
   - **有条件通过** → 调度 sdk-test-case-gen 按评审报告修订，完成后重新评审
   - **不通过** → 调度 sdk-test-case-gen 按评审报告修订，完成后重新评审

**失败处理**：
- 评审不通过/有条件通过 → 调度 sdk-test-case-gen SubAgent 重新生成用例（传入评审报告路径），完成后重新执行 sdk-case-review 评审（最多 2 轮重新评审）
...
本流程（测试设计 3 阶段）完成后，下游流水线中还有一个 **`sdk-case-confrontation`（安卓意图验证）** 阶段，该阶段：

1. **触发时机**：本阶段（sdk-test-design）完成后自动触发（不依赖 `sdk-implementation`，仅需 Android 原生 SDK 源码）
2. **核心任务**：对照 **Android 原生 SDK 源码**验证每条鸿蒙测试用例的设计意图是否合理
3. **判定三态**：`keep`（意图合理保持不动）、`modify_test_case`（意图有偏差需修改）、`delete_test_case`（无对应意图需删除）
4. **修订机制**：验证前将 `04-test-cases.json` 复制为 `04-test-cases-revised.json`，修改/删除写在此文件上，原始文件不动
5. **产物**：`06-confrontation-report.md`（含每条用例的 Android 意图分析）+ `06-confrontation.json`（结构化数据）+ `04-test-cases-revised.json`（修订后的测试用例）

详细流程见 `../../agent-android-sdk/.claude/prompts/test/06-sdk-case-confrontation.md`。
...
| 安卓意图验证 Prompt | `.claude/prompts/test/06-sdk-case-confrontation.md` |

---

*本文档最后更新：2026-06-16（v5.0 - 下游阶段更名为安卓意图验证）*
