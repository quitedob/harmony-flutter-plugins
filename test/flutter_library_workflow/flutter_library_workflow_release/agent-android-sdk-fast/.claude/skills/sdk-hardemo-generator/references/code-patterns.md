...
```typescript
// 步骤3 输入型动作：正则表达式参数 → Text+TextInput；匹配动作 → Button
Text('正则表达式')
  .fontSize(14)
...
Button('匹配')    // 按钮label取动作浓缩（如'匹配'），不得写无关文案
  .id('btn_match')
  .onClick(() => {
...
```typescript
// 步骤3 选择型动作：类型选择 → Text+Select；创建动作 → Button
Text('类型选择')
  .fontSize(14)
  .fontWeight(FontWeight.Medium)
  .margin({ bottom: 4 })
...
**示例代码八，点击按钮并观察**
```typescript
// 步骤：触发型动作 → Button('查看可筛选信号')
// 观察点（来自checkpoint）：信号列表 → result/预览区展示
...
**示例代码十二，逻辑 API + 视觉组件组合渲染（DiffAdapter→DiffList）
```typescript
// 场景：用例 checkpoint/expected_result 含"渲染/显示"语义，但 coveredApis 是逻辑类（DiffAdapter），本身不渲染。Demo 须组合 HAR 视觉组件（DiffList）完成渲染，不能只 dump `getDatas()` 到 result。

import { BaseMutableData, DiffAdapter, DiffList, ItemBuilderParam } from 'diffadapter';

class DemoData extends BaseMutableData {
  id: number = 0;
  name: string = '';
  getItemViewId(): number { return 1; }
  uniqueItemFeature(): Object { return this.id; }
  areUISame(n: BaseMutableData): boolean { return this.name === (n as DemoData).name; }
}

@Entry
@Component
struct ComboRenderPage {
  @State result: string = '等待操作...';
  @State dataList: BaseMutableData[] = [];   // 视觉消费数据源（必须 @State）
  private adapter: DiffAdapter = new DiffAdapter();  // 逻辑 API 承载者
  private nextId: number = 1;

  // 视觉渲染单元：每个 item 如何显示
  @Builder
  itemBuilder(item: ItemBuilderParam) {
    Row() {
      Text('#' + (item.data as DemoData).id)
        .fontSize(14).fontColor('#999999').width(40)
      Text((item.data as DemoData).name)
        .fontSize(14).fontColor('#333333').layoutWeight(1)
    }
    .padding(12).width('100%').backgroundColor(Color.White).borderRadius(4)
  }
    ...
    Column() {
      ...
      Scroll() {
        Column() {
          ...
          // Actions 区：按钮调用逻辑 API，同步 dataList 驱动视觉组件刷新
          Button('新增数据')
            .onClick(() => {
              let d: DemoData = new DemoData();
              d.id = this.nextId++; d.name = '项 ' + d.id;
              this.adapter.addData(d);
              this.dataList = this.adapter.getDatas();   // 同步给视觉消费组件
              this.result = 'addData → 总数 ' + this.adapter.getItemCount();
            })

          // 渲染预览区：视觉组件消费逻辑 API 产出的数据
          Text('DiffList 预览区').fontSize(14).fontWeight(FontWeight.Medium).margin({ bottom: 4 })
          DiffList({
            dataList: this.dataList,
            itemBuilder: (item: ItemBuilderParam): void => { this.itemBuilder(item); }
          })
            .width('100%').height(200)
        }
      }.layoutWeight(1)
      ResultPanel({ result: this.result }).height(120)
    }
  }
}

// 要点：
// 1. `adapter`（逻辑 API）产出数据，`@State dataList`（视觉数据源）须用 @State，`DiffList`（视觉组件）消费渲染；
// 2. 每次逻辑操作后 `this.dataList = this.adapter.getDatas()` 同步，DiffList 自动刷新；
// 3. result 文本记录操作结果，预览区展示渲染产物——两者配合，缺一不可。
```
