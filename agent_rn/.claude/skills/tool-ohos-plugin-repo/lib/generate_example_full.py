...
def read_ohos_library_package_name():
    ...
    js-only 模块无 harmony/library 目录，返回 None。
    """
    oh_pkg_path = os.path.join(os.getcwd(), 'ohos', 'harmony', 'library', 'oh-package.json5')
...
def rewrite_harmony_oh_package_har_paths(example_root: str) -> None:
    ...
    使用 example 根目录的绝对路径（Windows 短路径场景下为真实目录 D:\\rn\\N\\example）。
    """
...
def update_example_package_json(target_path, package_name, tgz_file_name):
    """更新 example/package.json，添加本地依赖（tgz 使用相对路径 ../）"""
...
def copy_library_directory(target_path):
    """拷贝 library 目录到目标位置"""
    source_library_path = os.path.join(os.getcwd(), 'ohos', 'harmony', 'library')
...
def update_entry_oh_package(target_path, ohos_package_name, npm_package_name):
    """更新 entry/oh-package.json5，添加 library 依赖（指向 tgz 包中的 HAR）"""
    ...
    # 构建 node_modules 路径：entry 在 harmony/entry/，node_modules 在根目录
    # 相对路径：../../node_modules/<npm_package_name>/harmony/library.har
    library_path = f"../../node_modules/{npm_package_name}/harmony/library.har"
    
    # 检查是否已存在该依赖，避免重复添加
    if f'"{ohos_package_name}"' in entry_oh_package_content:
        print(f'  已存在依赖 {ohos_package_name}，跳过添加')
        return
    ...
    if '"dependencies": {' in entry_oh_package_content:
        entry_oh_package_content = entry_oh_package_content.replace(
            '"dependencies": {',
            f"\"dependencies\": {{\n    \"{ohos_package_name}\": \"file:{library_path}\","
        )
...
def update_cmake_lists(target_path, ohos_package_name, npm_package_name):
    ...
    # 检查是否已存在该依赖，避免重复添加
    if ohos_package_name in cmake_lists_content:
        print(f'  已存在 CMake 配置 {ohos_package_name}，跳过添加')
        return
    
    # 添加 library 目录和链接（从 node_modules 中的 tgz 包）
    cmake_add_subdirectory = f'add_subdirectory("${{OH_MODULE_DIR}}/{ohos_package_name}/src/main/cpp" ./library)'
    
    # 从 node_modules/<npm_package_name>/harmony/library 提取库名
    library_cmake_path = os.path.join(target_path, 'node_modules', npm_package_name, 'harmony', 'library', 'src', 'main', 'cpp', 'CMakeLists.txt')
    library_name = 'library'  # 默认值
...
def find_package_classes(target_path, npm_package_name):
    """在 node_modules/<npm_package_name>/harmony/library/src/main/cpp 目录下递归查找继承自 public Package 的类"""
    package_classes = []
    library_cpp_path = os.path.join(target_path, 'node_modules', npm_package_name, 'harmony', 'library', 'src', 'main', 'cpp')
...
def generate_package_provider(target_path, npm_package_name):
    ...
    package_classes = find_package_classes(target_path, npm_package_name)
...
def find_rn_package_classes(target_path, npm_package_name):
    """在 node_modules/<npm_package_name>/harmony/library/src/main/ets 目录下递归查找实现了 RNPackage 的类"""
    rn_package_classes = []
    library_ets_path = os.path.join(target_path, 'node_modules', npm_package_name, 'harmony', 'library', 'src', 'main', 'ets')
        ...
        for file in files:
            if file.endswith('.ets') or file.endswith('.ts'):
                ...
                for class_name in matches:
                    ...
                    relative_path = os.path.relpath(file_path, os.path.join(target_path, 'node_modules', npm_package_name, 'harmony', 'library', 'src', 'main', 'ets'))
                    # 移除扩展名（.ets / .ts）
...
def generate_rn_package_factory(target_path, npm_package_name, ohos_package_name):
    ...
    rn_package_classes = find_rn_package_classes(target_path, npm_package_name)
    ...
    pkg_import_entry = resolve_ohos_import_entry(target_path, ohos_package_name)

    # 删除该 package 下"非当前入口"的 import（例如 from '@react-native-community/netinfo/NetInfoPackage'）
    existing_content = re.sub(
        rf'^\s*import\s+\{{[^}}]+\}}\s+from\s+[\'"]{re.escape(ohos_package_name)}/(?!{re.escape(pkg_import_entry.split("/", 1)[1])}\b)[^\'"]+[\'"];\s*$\n?',
        '',
        existing_content,
        flags=re.MULTILINE
    )
...
def find_fabric_component_structs(target_path: str, npm_package_name: str) -> list:
    ...
    优先扫描 node_modules/<npm_package_name>/harmony/library/src/main/ets/components/*.ets 的 export struct；
    若存在旧版 ts.ts 中 export * from .../components/... 仍可作为补充（新版 ts.ts 不再 export .ets）。
    """
    library_root = os.path.join(target_path, 'node_modules', npm_package_name, 'harmony', 'library')
    names: list = []
    seen: set = set()
...
def main():
    ...
    # 读取 OHOS 包名（来自 harmony/library/oh-package.json5），js-only 返回 None
    ohos_package_name = read_ohos_library_package_name()
    is_js_only = ohos_package_name is None
    
    if is_js_only:
        print('[info] js-only 模块检测：无 harmony/library，仅执行步骤 1、2、9、10')
    ...
    for step in sorted(set(requested_steps)):
        ...
        if step == 2:
            ...
            update_entry_oh_package(target_path, ohos_package_name, npm_package_name)
        elif step == 6:
            print('\n6. 修改 CMakeLists.txt...')
            update_cmake_lists(target_path, ohos_package_name, npm_package_name)
        elif step == 7:
            print('\n7. 生成 PackageProvider.cpp...')
            generate_package_provider(target_path, npm_package_name)
        elif step == 8:
            print('\n8. 生成 RNPackagesFactory.ets，并注册 Fabric 组件（Index.ets）...')
            generate_rn_package_factory(target_path, npm_package_name, ohos_package_name)
