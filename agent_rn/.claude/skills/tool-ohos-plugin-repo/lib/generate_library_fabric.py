...
步骤一览（与位置参数对应）：
  ...
  4  根据 codegen 产物生成 Fabric 组件 ArkTS 实现模板（harmony/library/src/main/ets/components/*.ets）
  5  生成 GeneratedPackage.ts（extends RNPackage，用于注册 Fabric DescriptorWrapper）
  6  同步 harmony/library/oh-package.json5 的 name 与根 package.json
  7  重写 harmony/library/ts.ts 导出（仅 GeneratedPackage；不 export components/*.ets）
...
def step_02_copy_harmony_template(root: str, dry_run: bool) -> None:
    print("\n[步骤 2] 拷贝 harmony 模板到项目根")
    common.copy_harmony_template(root, dry_run)


def step_03_npm_run_codegen(root: str, dry_run: bool) -> None:
    print("\n[步骤 3] npm run codegen")
...
def step_05_generate_generated_package(
    ...
    _ = plan
    print("\n[步骤 5] 生成 GeneratedPackage.ts（extends RNPackage，注册 Fabric DescriptorWrapper）")
...
def step_06_sync_oh_package_name(root: str, dry_run: bool) -> None:
    print("\n[步骤 6] 同步 harmony/library/oh-package.json5 → name")
    common.sync_harmony_library_oh_package_name(root, dry_run)


def step_07_write_library_ts(
    root: str,
    plan: common.GenerationPlan,
    ets_dir: str,
    dry_run: bool,
    ...
    _ = plan
    print("\n[步骤 7] 重写 harmony/library/ts.ts（仅 GeneratedPackage；components 由应用侧按路径引用）")
...
def run_step(
    ...
    if step == 2:
        step_02_copy_harmony_template(root, dry_run)
    elif step == 3:
        step_03_npm_run_codegen(root, dry_run)
    elif step == 4:
        ...
        step_06_sync_oh_package_name(root, dry_run)
    elif step == 7:
        step_07_write_library_ts(root, plan, ets_dir, dry_run)
    else:
...
def main() -> None:
    ...
    for step in sorted(set(requested)):
        run_step(step, root, ets_dir, args.dry_run, plan_holder)
